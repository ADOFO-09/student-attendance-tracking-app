    ...
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
      <div class="inline-flex gap-x-2">
            <select wire:model="year"
                ...
                {{-- Create an option for each year in the loop --}}
                    <option value="{{ $y }}">{{ $y }}</option>
                @endforeach
            </select>

            <select wire:model="month"
                class="py-3 px-3 pe-9 block w-full border-gray-200 rounded-full text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                <option selected="">Select Month</option>
                {{-- loop from 1 to 12 --}}
                @foreach (range(1, 12) as $m)
                    {{-- Create an option for each month in the loop --}}
            ...
            <select wire:model="grade"
                ...
                {{-- Assuming $grades is an array of grade objects with id and name properties --}}
                @foreach ($grades as $grade)
                {{-- Create an option for each grade --}}
            ...
            @if ($year && $month && $grade)
        <button type="button" wire:click="exportToExcel()" class="py-3 px-3 flex justify-center items-center size-11 text-sm font-medium rounded-lg border border-transparent bg-green-600 text-white hover:bg-green-700 focus:outline-hidden focus:bg-green-700 disabled:opacity-50 disabled:pointer-events-none">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
